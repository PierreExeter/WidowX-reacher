...
class EvalCallback(EventCallback):
    ...
    def __init__(self, eval_env: Union[gym.Env, VecEnv],
        ...
        # added by Pierre
        if not isinstance(eval_env, VecEnv) and not isinstance(eval_env, stable_baselines.her.HERGoalEnvWrapper):
            eval_env = DummyVecEnv([lambda: eval_env])
            assert eval_env.num_envs == 1, "You must pass only one environment for evaluation"
        elif isinstance(eval_env, stable_baselines.her.HERGoalEnvWrapper):
            print("Not replacing HERGoalEnvWrapper env by a DummyVecEnv")

        # if not isinstance(eval_env, VecEnv):
        #     eval_env = DummyVecEnv([lambda: eval_env])
        # assert eval_env.num_envs == 1, "You must pass only one environment for evaluation"

        print("EVAL ENV TYPE : ", eval_env)
