...
sampler="tpe"
pruner="median"
log_dir="logs/opti10t_0.01M_widowx_reacher-v6/"
env="widowx_reacher-v6"
...
echo "HER OPTI"
python3 2_train.py --algo her --env ${env} -n ${nsteps} --log-folder ${log_dir} -optimize --n-trials ${ntrials} --n-jobs ${njobs} --sampler ${sampler} --pruner ${pruner}  &> submission_log/log_her_td3_opti.run
# python3 2_train.py --algo her --env ${env} -n ${nsteps} --log-folder ${log_dir} -optimize --n-trials ${ntrials} --n-jobs ${njobs} --sampler ${sampler} --pruner ${pruner}  &> submission_log/log_her_sac_opti.run
# python3 2_train.py --algo her --env ${env} -n ${nsteps} --log-folder ${log_dir} -optimize --n-trials ${ntrials} --n-jobs ${njobs} --sampler ${sampler} --pruner ${pruner}  &> submission_log/log_her_ddpg_opti.run
