...
class WidowxEnv(gym.Env):
    ...
    def _get_reward(self, goal):
        ...
        # added by Pierre
        self.dist_to_goal = np.linalg.norm(self.current_pos[:3] - goal)

        self.dense_reward = - (self.dist_to_goal ** 2)

        self.sparse_reward = 0
        if self.dist_to_goal <= 0.0005:
            self.sparse_reward += 1

        return (self.dense_reward + self.sparse_reward)
